...
public class SynReportCardTranx implements ReportUtil {
	...
	public Object setRequest(HttpServletRequest objRequest,
		...
		try {
			...
			if (mode.trim().equals("Search") || mode.trim().equals("Next") || mode.trim().equals("View") || mode.trim().equals("Prev") || mode.trim().equals("Down")) {
				...
				if (objRequest.getParameter("TRANX_TYPE") != null) {
					objReportViewBean.setTranxType(objRequest.getParameter("TRANX_TYPE"));
				}
	...
	public StringBuffer getReportFields() {
		...
		sbfStr.append("TO_CHAR(TL.DATETIME, 'hh24:mi:ss') AS TRANX_TIME, TO_CHAR(TL.DATETIME, 'DD/MM/YYYY') AS TRANX_DATE, ");
		sbfStr.append("TL.TERMINALID, TL.MERCHANTID, TL.MERCHANTNAME, TL.CARDNUMBER, TL.REFNO, ");
		sbfStr.append("TL.TRACENO, TL.TRANXCODE, TL.AMOUNT, TL.RESPONSECODE, ");
		sbfStr.append("TL.APPROVALCODE, CU.CUSTOMER_NAME ");
	...
	public StringBuffer getFilterFields(Object obj) {
		...
		if ((tranxType != null) && (!tranxType.trim().equals(""))) {
			sbfStr.append("AND TL.TRANXCODE = '" + tranxType + "' ");
		}
