...
<html:html>
   ...
   <body bgcolor="ffffff">
      <html:form action="searchcardbatchauthaction">
      ...
      <div style="padding-top: 5px;"><logic:present name="CHKTOTALRECS" scope="request">
      <b><bean:write name="cardBatchSearchForm"
         property="total" /></b>
      <bean:message key="cardBatch.total" /></div>
      </logic:present>
      <%-- <%
         if (Integer.valueOf(request.getAttribute("TOTAL")
         					.toString()) > 10) {
         %> --%>
      ...
      <%
         if (((CommonDataBean) pageContext
         						.getAttribute("appProcess")).getColumn3()
         						.equals("A")) {
         %>
      <form name="listForm" action="cardbatchprocessdispatchaction.do">
      <%
         } else {
         %>
      <form name="listForm" action="cardbatchauthdispatchaction.do">
      <html:hidden
         property="authForm" value="auth" />
      <%
         }
      ...
      <html:hidden property="issuerId"
         ...
         property="method" value="change" /> <html:submit
         value="Authorized" />
      </form> </display:column>
      </display:table>
      </logic:present>
