...
import org.transinfo.cacis.formbean.batchprocess.RenewalBatchProcessSuccess;
import org.transinfo.cacis.formbean.batchprocess.RenewalCardBPForm;
import org.transinfo.cacis.util.DateUtil;

@SuppressWarnings( { "unchecked", "deprecation" })
public class RenewalCardsDispatchAction extends BaseDispatchAction {
	...
	public ActionForward List(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			int totApps = objCardBatchProcessManager
					.getTotalRenewalCardsApps(issuerId);
			...
			System.out
					.println("Error converting to form bean in RenewalCardsDispatchAction : "
							+ e.getMessage());
			throw new TPlusException(
					"Error converting to form bean in RenewalCardsDispatchAction : "
							+ e);
		}
		...
		try {
			...
			if (isError) {
				...
				if(objSystemParamDto == null){
					...
					Collection applicationsList = objCardBatchProcessManager.listReplacementCardApps(
							issuerId, pageNo);
				...
				ArrayList allApps = objCardBatchProcessManager
						.getAllRenewalApplication(issuerId);
				...
				while (itr.hasNext()) {
					CardsRenewalDto objCardsRenewalDto = (CardsRenewalDto) itr
							.next();

					CardsDto objCardsDto = objCardManager.getCard(String
							.valueOf(objCardsRenewalDto.getCardNumber()));
					CardProductDto objCardProductDto = objCardProductManager
							.getCardProductDto(objCardsDto.getCardProductId());

					ApplicationProcessDto objApplicationProcessDto = objCardBatchProcessManager
							.getCustomerById(objCardsDto.getCustomerId());
					...
					try {
						if (objApplicationProcessDto != null) {
							...
							objApplicationProcessDto.setUserId(objForm
									.getUserId());

							boolean appProcessRes = objCardBatchProcessManager.renewalBatchProcess(objApplicationProcessDto, objCardsDto, indexesList, objCardProductDto, objCardsRenewalDto, objForm.getBatchId(), userId, extendInterval, oldExpDate);
							if (appProcessRes) {
								count++;
								...
								genAppsIds.append("'");
								genAppsIds.append(objCardsRenewalDto
										.getCardRenewalSerial());
								genAppsIds.append("',");

								newCardsNos.append("'");
								newCardsNos.append(String
										.valueOf(objCardsRenewalDto
												.getCardNumber()));
								newCardsNos.append("',");

								objRenewalBatchProcessSuccess = new RenewalBatchProcessSuccess();
								objRenewalBatchProcessSuccess
										.setApplicationId(objCardsRenewalDto
												.getCardRenewalSerial());
								objRenewalBatchProcessSuccess
										.setCustomerName(objApplicationProcessDto
												.getCustomerName());
								objRenewalBatchProcessSuccess
										.setOldCardNumber(String
												.valueOf(objCardsRenewalDto
														.getCardNumber()));
								objRenewalBatchProcessSuccess
								...
								objRenewalBatchProcessSuccess
										.setAppliedDate(DateUtil
												.convertDateToDateString(objCardsRenewalDto
														.getUpdatedDate()));
				...
				if (successApps.size() > 0) {
					...
					if (overallSuccess) {
						...
						objForm.setAppIds(genAppsIds.toString().substring(0,
								(genAppsIds.toString().length() - 1)));
						objForm.setNewCardNos(newCardsNos.toString().substring(
								0, (newCardsNos.toString().length() - 1)));
