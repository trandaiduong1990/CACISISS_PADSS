...
public class SearchCardEmbossingAction extends BaseAction {

	private Logger logger = Logger.getLogger(SearchCardEmbossingAction.class);

	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws TPlusException, Exception {

		logger.error("inside SearchCardEmbossingAction");
		...
		String testVariable = objForm.getSerialNosToEmboss();
		objForm.setSelectedEmbossSerialNos(testVariable.split(","));
		...
		Collection cardEmbossingList = objManager.search(objDto, pageNo);
		
		/*String selectCardEmbossSerialNos[] = objForm.getSelectedEmbossSerialNos();
		if(selectCardEmbossSerialNos != null && selectCardEmbossSerialNos.length > 0){
			objForm.setSelectedEmbossSerialNos(selectCardEmbossSerialNos);
		}*/
