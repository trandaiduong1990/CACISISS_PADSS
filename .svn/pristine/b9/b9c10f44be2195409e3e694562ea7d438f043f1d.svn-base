...
public class Renewal implements ReportUtil {
	...
	public StringBuffer getReportFields() {
		...
		sbfStr.append("CA.CARDNUMBER, CU.CUSTOMER_NAME, CP.CARDPRODUCTNAME, CA.EXPDATE, CAS.DESCRIPTION, CU.EMAIL, CUA.TEL ");
	...
	public StringBuffer getFilterFields(Object obj) {
		...
		StringBuffer sbfStr = new StringBuffer();
		sbfStr.append("FROM CARDS CA, CUSTOMER CU, CARD_PRODUCTS CP,  CARD_STATUS CAS, CUSTOMER_ADDRESS CUA, SERVER_PARAMETERS SP ");
		...
		sbfStr.append("AND CU.CUSTOMER_ID = CUA.CUSTOMER_ID ");
		sbfStr.append("AND TO_DATE(CA.EXPDATE, 'MMYY') <= ADD_MONTHS(SYSDATE, SP.RENEWAL_TIME_INTERVAL) ");
		
		if ((cardProduct != null) && (!cardProduct.trim().equals(""))) {
			sbfStr.append("AND CA.CARD_PRODUCT_ID = '" + cardProduct + "' ");
		}

		sbfStr.append("ORDER BY CAS.DESCRIPTION DESC ");
