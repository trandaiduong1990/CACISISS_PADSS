...
public class CardProductManager {

	private Logger logger = Logger.getLogger(CardProductManager.class);

	private CardProductDAO objDAO;

	public CardProductManager() throws Exception {
		objDAO = DAOFactory.getInstance().getCardProductDAO();
	...
	public CardProductDto getCardProductDto(String cardProductId)
		...
		try {
			objDto = objDAO.getCardProductDto(cardProductId);
	...
	public boolean validate(Object obj, int mode) throws TPlusException {
		...
		CardProductDto objCardProduct = (CardProductDto) obj;

		System.out.println(objCardProduct.getCardProductId());

		if (mode == 0 && objDAO.checkExistrecord(objCardProduct) > 0) {
			rtnMessage = false;
		}
		if (mode == 1 && objDAO.checkExistrecord(objCardProduct) == 0) {
			rtnMessage = false;
		}
