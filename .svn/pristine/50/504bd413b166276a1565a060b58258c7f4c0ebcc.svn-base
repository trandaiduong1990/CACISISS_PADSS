...
public class PhotocopyRequestDAOImpl extends BaseDAOImpl implements
		PhotocopyRequestDAO {
	...
	public boolean update(OriginalRequestDto objOriReqDto)
		...
		try {
			Session session = HibernetFactory.currentSession();
			tx = session.beginTransaction();
			if (objOriReqDto.getStatus() == 1) {
				objOriReqDto.setStatus(2);
				...
				UpdateCount = session.createQuery(UpdateSql).setInteger(
						...
						.executeUpdate();
				// session need to be closed to allow it be used in
				// getOriginalRequest
				session.flush();
				tx.commit();
				HibernetFactory.closeSession();
				CardActivityDto objInsertDto = new CardActivityDto();
				OriginalRequestDto objGetDto = new OriginalRequestDto();
				objGetDto = (OriginalRequestDto) getOriginalRequest(objOriReqDto
						.getSettlementId());
				// open the session back ...
				session = HibernetFactory.currentSession();
				tx = session.beginTransaction();
				objInsertDto.setCardNumber(Long.parseLong(objGetDto
						.getCardNumber()));
				objInsertDto.setDateTime(objGetDto.getTranxDate());
				objInsertDto.setStationIp("000.000.000.000");
				objInsertDto.setActivity("Photocopy Request");
				objInsertDto.setUserId(objOriReqDto.getUserId());
				objInsertDto.setLastUpdatedBy(objGetDto.getIssuerId());
				Date today = new Date();
				objInsertDto.setUpdatedDate(today);
