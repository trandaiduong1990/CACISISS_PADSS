...
public class BatchProcessNewProductCards implements ReportUtil {
	...
	public StringBuffer getReportFields1() {
		// number of fields should be lessthan or equal 20.
		StringBuffer sbfStr = new StringBuffer();
		sbfStr.append("SELECT APF.SNO AS APPID, APF.CUSTOMER_NAME AS CUSTNAME, ");
		sbfStr.append("APF.NRIC AS IDNO, TO_CHAR(C.CARDNUMBER) AS CARDNO, substr(C.EXPDATE,1,2)||'/'||substr(C.EXPDATE,3,4) AS EXPD, 'Primary Card' AS CARDTYPE ");
	...
	public StringBuffer getReportFields2() {
		// number of fields should be lessthan or equal 20.
		StringBuffer sbfStr = new StringBuffer();
		sbfStr.append("SELECT APF.APPLICATION_ID AS APPID, APF.SUPPL_CUSTOMER_NAME AS CUSTNAME, ");
		sbfStr.append("APF.SUPPL_ID_NUMBER AS IDNO, TO_CHAR(C.CARDNUMBER) AS CARDNO, substr(C.EXPDATE,1,2)||'/'||substr(C.EXPDATE,3,4) AS EXPD, 'Supplementry Card' AS CARDTYPE ");
	...
	public StringBuffer getFilterFields1(Object obj) {
		...
		StringBuffer sbfStr = new StringBuffer();
		sbfStr.append("FROM CARDS C, CUSTOMER CU, ADDPRODUCT_FORMS APF ");
		sbfStr.append("WHERE C.CARDHOLDER_TYPE = 1 ");
		sbfStr.append("AND C.CUSTOMER_ID = CU.CUSTOMER_ID ");
		sbfStr.append("AND CU.CUSTOMER_ID = APF.CUSTOMER_ID ");
		sbfStr.append("AND C.CARDNUMBER IN (" + CardNo + ") ");
