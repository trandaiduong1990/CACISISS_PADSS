...
public class OriginalRequestDAOImpl extends BaseDAOImpl implements
		OriginalRequestDAO {
	...
	public boolean update(OriginalRequestDto objOriReqDto)
		...
		try {
			Session session = HibernetFactory.currentSession();
			tx = session.beginTransaction();
			if (objOriReqDto.getStatus() == 0) {
				objOriReqDto.setStatus(1);
				...
				objInsertDto.setActivity("Original Request");
