...
public class ExpiredCardList implements ReportUtil {
	...
	public Object setRequest(HttpServletRequest objRequest,
		...
		try {
			...
			if (mode.trim().equals("Search") || mode.trim().equals("Next") || mode.trim().equals("View") || mode.trim().equals("Prev") || mode.trim().equals("Down")) {
				...
				if (objRequest.getParameter("EXP_DATE") != null) {
					objReportViewBean.setExpireDate(objRequest.getParameter("EXP_DATE"));
				}
	...
	public StringBuffer getReportFields() {
		...
		sbfStr.append("CU.CUSTOMER_NAME, TO_CHAR(CU.DOB, 'DD/MM/YYYY') AS DOB, ");
		sbfStr.append("CU.ID_NUMBER, CA.CARDNUMBER, CA.EXPDATE, CP.CARDPRODUCTNAME, CAS.DESCRIPTION ");
	...
	public StringBuffer getFilterFields(Object obj) {
		...
		String expDate = objReportViewBean.getExpireDate();
