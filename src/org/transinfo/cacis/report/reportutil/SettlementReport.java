...
public class SettlementReport implements ReportUtil {
	...
	public StringBuffer getReportFields() {
		...
		sbfStr.append("SELECT TO_CHAR(DATETIME, '" + OUTPUT_DATE_FORMAT
				+ "') AS DATETIME, SE.CARDNUMBER, TRANXCODE, TRANXDATE,");
		sbfStr
				.append("MERCHANTNAME, MERCHANTCITY, MERCHANTCOUNTRY,APPROVALCODE, AMOUNT_CR, AMOUNT_DR ");
		return sbfStr;
	}
	...
	public StringBuffer getFilterFields(Object obj) {
		...
		StringBuffer sbfStr = new StringBuffer();
		sbfStr.append(" FROM SETTLEMENTS SE, CARDS CA");
		...
		sbfStr.append(" AND (DATETIME <TO_DATE('"
				...
				+ "') + 1) ");
		sbfStr.append("AND SE.CARDNUMBER = CA.CARDNUMBER ");

		if (!IssuerId.trim().equals("")) {
			sbfStr.append(" AND CA.ISSUER_ID = '" + IssuerId + "'");
		}
		if (!CardNo.trim().equals("")) {
			sbfStr.append(" AND SE.CARDNUMBER = '" + CardNo + "'");
		}

		sbfStr
				.append(" GROUP BY SE.CARDNUMBER, TRANXDATE, DATETIME, TRANXCODE, APPROVALCODE, MERCHANTNAME, MERCHANTCITY, MERCHANTCOUNTRY, AMOUNT_CR, AMOUNT_DR ");
		sbfStr
				.append(" ORDER BY SE.CARDNUMBER, TRANXDATE, DATETIME, TRANXCODE, APPROVALCODE, MERCHANTCOUNTRY, MERCHANTCITY, MERCHANTNAME  ");
		return sbfStr;
	}
