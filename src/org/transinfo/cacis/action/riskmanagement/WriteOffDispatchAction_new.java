...
public class WriteOffDispatchAction_new extends BaseDispatchAction {
	...
	public ActionForward add(ActionMapping mapping, ActionForm form,
		...
		// Form Validations
		/*
		 * errors = objForm.validate(mapping, request); if (errors != null &&
		 * !errors.isEmpty()) { saveErrors(request, errors);
		 * request.setAttribute("ACTION", "add"); return
		 * mapping.getInputForward(); }
		 */
		WriteOffForm objValidateForm = new WriteOffForm();
		objValidateForm = validateRadio(objForm);
		// DTO Creation
		try {
			BeanUtils.copyProperties(objDto, objValidateForm);
		} catch (Exception e) {
	...
	public WriteOffForm validateRadio(WriteOffForm objForm) {
		WriteOffForm objValidatedForm = new WriteOffForm();
		try {
			BeanUtils.copyProperties(objValidatedForm, objForm);
		} catch (Exception e) {
			System.out.println("Error converting to form bean: " + e);
		}
		if (objForm.getDpd0() == null) {
			objValidatedForm.setDpd0("-1");
		}
		if (objForm.getDpd0() == null) {
			objValidatedForm.setDpd30("-1");
		}
		if (objForm.getDpd0() == null) {
			objValidatedForm.setDpd60("-1");
		}
		if (objForm.getDpd0() == null) {
			objValidatedForm.setDpd90("-1");
		}
		if (objForm.getDpd0() == null) {
			objValidatedForm.setDpd120("-1");
		}
		return objValidatedForm;
	}
}
