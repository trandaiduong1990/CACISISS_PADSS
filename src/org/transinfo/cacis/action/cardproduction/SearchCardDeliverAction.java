...
public class SearchCardDeliverAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		// this for card received link directly getting the cards list
		String Success = "success";
		if (request.getParameter("status") != null) {
			objForm.setCardDeliverStatus(Integer
					.toString(CommonCodes.CARD_PROCESS_CREATED));
			Success = "receivesucc";
		}
		try {
		...
		Collection carDeliverList = objManager.search(objDto, pageNo);

		if (carDeliverList.size() > 0) {

			request.setAttribute("SEARCHLIST", carDeliverList);
