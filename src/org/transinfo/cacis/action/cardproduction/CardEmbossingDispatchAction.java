...
import org.transinfo.cacis.formbean.cardproduction.CardEmbossingSearch;
...
public class CardEmbossingDispatchAction extends BaseDispatchAction {

	private Logger logger = Logger.getLogger(CardEmbossingDispatchAction.class);
	...
	throws TPlusException, Exception {
		...
		// Form Validations
		CardEmbossingSearch objForm = (CardEmbossingSearch) form;
		/*
		...
		//String fileName = "CARD_EMB_" + dateTime + ".txt";
		// Action Execution
		...
		String strSelectCardEmbossSerialNos = objForm.getSerialNosToEmboss();
		String selectCardEmbossSerialNos[] = strSelectCardEmbossSerialNos.split(",");
		// date for emboss table. to group by later for multiple downloads
		...
		for (int i = 0; i < selectCardEmbossSerialNos.length; i++) {
			...
			String cutomerId = "";
			try{
				logger.error("embossSerialNo :: "+embossSerialNo);
				// get CardEmboss object
				CardEmbossingDto objCardEmbossingDto = objManager.getCardEmboss(embossSerialNo);
				logger.error("objCardEmbossingDto :: "+objCardEmbossingDto.getCardEmbossId());
				// get Card object
				CardsDto objCardsDto = objManager.getCard(embossSerialNo);
				logger.error("objCardsDto :: "+objCardsDto.getCardNumber());
				// card number
				...
				logger.error("cutomerId :: "+cutomerId);
				// get Customer object
				ApplicationProcessDto objApplicationProcessDto = objManager.getCustomerByCusId(cutomerId);
				logger.error("objApplicationProcessDto :: "+objApplicationProcessDto.getCustomerId());
				// generate image path
				...
				track1data = StringUtil.RPAD(track1data, 79, " ");
				...
				track2data = StringUtil.RPAD(track2data, 41, " ");
				...
				objEmbossFileContent.setTrack2data(track2data);
				...
				if(embossFile){
					//rowNumber++;
					boolean updateRes = objManager.updateObjects(embossSerialNo, userId, embossDate);
					if(updateRes){
