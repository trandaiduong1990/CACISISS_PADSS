...
import org.transinfo.cacis.formbean.settings.BranchSearchForm;
import org.transinfo.cacis.formbean.settings.BranchSetupForm;

@SuppressWarnings( { "deprecation", "unchecked" })
public class SearchBranchAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		if ((request.getParameter("method") != null)
			...
			try {
				BranchSearchForm objForm = (BranchSearchForm) form;
				BranchSetupForm newObjForm = new BranchSetupForm();
			...
			// Dto Creation
			BranchSearchDto objDto = new BranchSearchDto();
			try {
				BeanUtils.copyProperties(objDto, objForm);
			} catch (Exception e) {
				throw new TPlusException(
						"Error converting to form bean in SearchBranchAction execute method: "
								+ e);
			}
			// Action Execution
			BranchManager objManager = new BranchManager();
			Collection brachRecords = objManager.search(objDto, pageNo);

			request.setAttribute("SEARCHLIST", brachRecords);
