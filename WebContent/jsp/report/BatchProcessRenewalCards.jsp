			...
			try {
				...
				if (session.getAttribute("ADMINDATABEAN") != null) {
					if (intErr != ErrorCodes.NO_RECORDS_FOUND) {
						...
						intTotal = intTotalNoOfRecords
								/ AdminConfig.intPaginationConstant
								+ (intTotalNoOfRecords
										% AdminConfig.intPaginationConstant > 0 ? 1
										: 0);
					}
				} else {
...
<table width="102%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<table border="0" cellspacing="0" cellpadding="2" width="100%">
			<tr>
				<%String reportName = "Transaction Listing Report";
			...
			<tr>
				...
				if (intArlSize != 0) {
					for (int intIndex = 0; intIndex < intArlSize; intIndex++) {
						Hashtable AcqDetails = (Hashtable) arlIssuer
								.get(intIndex);
						String zszAcqId = (String) AcqDetails.get("ISSUER_ID");
						String zszAcqName = (String) AcqDetails
								.get("ISSUER_NAME");
						zszSelectedText = "";
						if (strIssuerId != null
								&& strIssuerId.trim()
										.equalsIgnoreCase(zszAcqId))
						...
						%>
							...
							<%if (strReportCode.trim().equals("TND")
						|| strReportCode.trim().equals("TNL")) {
					System.out.println("...jsp_07...");
					DBUtil Xdbutil = new DBUtil();
					arlTranxCodes = Xdbutil.getTranxCodes();

					%>
							<tr>
								<td class="texts">Tranx Code:</td>
								<td><select name="TRANX_CODE">
					...
					if (intArlSize != 0) {
						for (int intIndex = 0; intIndex < intArlSize; intIndex++) {
							...
							%>
									<option value="<%=objArr[0]%>"
										<%if(strTranxCode.trim().equals(objArr[0])) out.print("selected");%>><%=objArr[1]%></option>
									<%}
					}
					...
					%>
							...
							<tr>
								...
								<td><input type="text" name="REF_NO" size="12" maxlength="12"
									value="<%=strRefNo%>"></td>
							</tr>
							<tr>
								<td class="texts">Response Code:</td>
								<td><input type="text" name="RESPONSE_CODE" size="6"
									maxlength="6" value="<%=strResponseCode%>"></td>
							</tr>
							<tr>
								<td class="texts">Approval Code:</td>
								<td><input type="text" name="APPROVAL_CODE" size="6"
									maxlength="6" value="<%=strApprovalCode%>"></td>
							</tr>
							<%}%>
							<tr>
								<td><input type="button"
									onClick="javascript:setmode(0,'Search','TNL','frmMain')"
									value="Search" name="Search"> <input type="button"
									onClick="javascript:setmode(4,'View','TNL')"
									value="PDF Download " name="PDF"></td>
							</tr>
						</table>
						</td>
					</tr>
